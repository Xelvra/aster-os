...
| B4 | qemu-test FAIL `open-failed` na `file.open("/README")`, reprodukovatelný i deterministický — připisováno „layout-sensitive" storage regresi (handoff H6, uzavřeno 2026-08-18) | **test disk znovu použit napříč běhy bez regenerace**: `testFileRemove` smaže `/README` z image, další běh na téže cestě správně hlásí `open-failed` (`debugfs` potvrdil chybějící README). Na stroji bez KVM (TCG) se navíc pomalost (4096B bloky = 8× 512B sektorové I/O) tváří jako hang a krátký `QEMU_TEST_TIMEOUT` ji ustřihne | **čerstvý disk těsně před každým během** (`make-test-disk.sh` dělá `rm -f "$OUT"`, ale jen když se spustí — nikdy nepoužívat starou cestu); na TCG dát `QEMU_TEST_TIMEOUT` ≥ ~600 s; `diag_verify_reads` (=dvojité čtení) běh cca zdvojnásobí | `QEMU_TEST_DISK=<cesta> tools/qemu-test.sh` dvakrát po sobě s tímtéž diskem → 2. běh očekávaně `open-failed`; s čerstvým diskem PASS (exit 99) |
