...
| C54 | `tools/make-test-disk.sh` bez `-b`: lokálně `mke2fs` zvolí 1024 B bloky pro 15 MiB ext2 a runtime testy projdou; v CI (jiná verze e2fsprogs) zvolí **4096 B** bloky pro identickou velikost disku a `zig build runtime-test` s tímto diskem **visí** (timeout) — reprodukováno lokálně vynucením `-b 4096`, hang se přesouvá mezi testy (jednou `persistent REPL history`, jednou `create/lookup/remove across a multi-block directory`), takže jde o skutečnou nekonečnou smyčku/deadlock v ext2 write cestě při 4096 B blocích, ne o pomalost. Přesná řádka v `ext2.zig`, kde smyčka nekončí, **nebyla dohledána** — jen workaround, ne kořenová příčina | `mke2fs`ova volba výchozí velikosti bloku není napříč verzemi/hosty deterministická; ADR-023 kontrakt „přesná mke2fs invokace" nikdy velikost bloku explicitně nezmiňoval, takže dřívější lokální testování vždy nevědomky běželo na 1024 B a bug se 4096 B bloky byl skrytý | `mke2fs -q -t ext2 **-b 1024** -O ^dir_index ...` — pinuje block size, disk je bit-identický nezávisle na hostu (ADR-014 duch); obchází bug, neopravuje ho v `ext2.zig` (možný budoucí handoff, pokud bude potřeba reálná podpora jiné velikosti bloku) | `QEMU_TEST_TIMEOUT=90 QEMU_TEST_DISK=... ./tools/qemu-test.sh` (přesně CI příkaz) PASS (exit 99) po přidání `-b 1024` |
