...
| B4 | qemu-test FAIL `open-failed` na `file.open("/README")`, reprodukovatelný i deterministický — připisováno „layout-sensitive" storage regresi (handoff H6, uzavřeno 2026-08-18) | **test disk znovu použit napříč běhy bez regenerace**: `testFileRemove` smaže `/README` z image, další běh na téže cestě správně hlásí `open-failed` (`debugfs` potvrdil chybějící README). Na stroji bez KVM (TCG) se navíc pomalost (4096B bloky = 8× 512B sektorové I/O) tváří jako hang a krátký `QEMU_TEST_TIMEOUT` ji ustřihne | **čerstvý disk těsně před každým během** (`make-test-disk.sh` dělá `rm -f "$OUT"`, ale jen když se spustí — nikdy nepoužívat starou cestu); na TCG dát `QEMU_TEST_TIMEOUT` s rezervou; po pinnutí bloku na 1024 B (C54) stačí v CI **90 s** (`.github/workflows/ci.yml`); hodnota ≥ ~600 s platila pro 4096B obrazy před opravou a dnes je potřeba jen při `diag_verify_reads` (dvojité čtení ≈ 2× doba běhu) | `QEMU_TEST_DISK=<cesta> tools/qemu-test.sh` dvakrát po sobě s tímtéž diskem → 2. běh očekávaně `open-failed`; s čerstvým diskem PASS (exit 99) |
...
| C45 | files: klik/Enter na běžný soubor shodil shell (`attempt to call a nil value (global 'is_read_only')`), lua-shell testy přitom prošly | **Lua lexikální scope lokálů:** `local function is_read_only` byla deklarovaná **až po** funkci `files_open_entry`, která ji volá — lokál je viditelný teprve od svého deklarování, takže se reference v dřívější funkci vyřešila na **globál = nil**. Dřív `is_read_only` volal jen `input.lua`, který v zkonkatenovaném chunku leží za deklarací, proto to fungovalo; přesun volání do dřívějšího `files.lua` to rozbil. Testy to nechytily, protože `files_open_entry` volaly jen na položku `..` (early return před touto větví) | funkci používající `local` helper definovat **až za ním** (pořadí v jednom zkonkatenovaném chunku), nebo helper přesunout výš | regresní test `files_open_entry` na běžný soubor i read-only `.bak` (obojí prošlo bez nil-global volání) |
...
| C54 | `tools/make-test-disk.sh` bez `-b`: lokálně `mke2fs` zvolí 1024 B bloky pro 15 MiB ext2 a runtime testy projdou; v CI (jiná verze e2fsprogs) zvolí **4096 B** bloky pro identickou velikost disku a `zig build runtime-test` s tímto diskem **visí** (timeout) — reprodukováno lokálně vynucením `-b 4096`, hang se přesouvá mezi testy (jednou `persistent REPL history`, jednou `create/lookup/remove across a multi-block directory`), takže se zprvu jevil jako nekonečná smyčka/deadlock v ext2 write cestě při 4096 B blocích. Přesná řádka v `ext2.zig`, kde smyčka nekončí, nebyla dohledána | **Překonáno H6 (uzavřeno 2026-08-18):** jde o artefakt **opakovaně použitého test disku** + **pomalost bez KVM**, ne o bug v `ext2.zig`. `testFileRemove` při PASS běhu smaže `/README`; opakovaný běh na stejném disku pak správně selže (`open-failed`), a bez KVM (TCG) je 4096 B blok (8× 512 B sektorové I/O) + `testDirMultiBlock` (~340 operací) extrémně pomalé, takže timeout vypadá jako hang. Na **čerstvém disku** 4096 B bloky procházejí bez smyčky (`allocBlock`/`ensureIndirect` lineárně, ne cyklicky; `diag_verify_reads` nikdy VIO-DIFF/VIO-STALE). mke2fsova volba výchozí velikosti bloku navíc není napříč verzemi/hosty deterministická | `mke2fs -q -t ext2 **-b 1024** -O ^dir_index ...` — pinuje block size, disk je bit-identický nezávisle na hostu (ADR-014 duch); zároveň **vždy čerstvý disk před každým během** (lekce H6) | `QEMU_TEST_TIMEOUT=90 QEMU_TEST_DISK=... ./tools/qemu-test.sh` (přesně CI příkaz) PASS (exit 99) po přidání `-b 1024`; 2026-08-19 navíc čerstvý 4096 B disk (`-b 4096`) PASS 3× opakovaně |
