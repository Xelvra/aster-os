...
(`./tools/capture-boot.sh --check`). A sample is shown on
[xelvra.github.io/aster-os](https://xelvra.github.io/aster-os/).
...
| M7* | Runtime | 431 KiB | ≈ 150 ms |

Boot times from `tools/bench.sh` (measured 2026-08-15): the M0–M3 rows are
approximated kernel-only times (the bootloader is subtracted); M4+ are
kernel-only on KVM. The current kernel is **431 KiB** (see `spec/roadmap.md`
...
and parsing hardening), so they are not directly comparable with the
optimized M0–M6 rows.
