...
/// current button state and the wheel delta (0 for a 3-byte packet).
...
/// Decode a 4-byte PS/2 mouse packet (Intellimouse: relative movement,
/// buttons and the wheel byte in Z). Pure, host-testable: no IRQ, no I/O.
/// Returns null when the packet is not a valid start (bit 3 of byte 0 is
/// always set) or the delta overflowed (bits 6/7). The wheel byte is signed:
/// positive = wheel up, negative (two's complement) = wheel down.
pub fn decodeMousePacket4(packet: *const [4]u8) ?MouseEvent {
    const b0 = packet[0];
    if (b0 & 0x08 == 0) return null; // out of sync
    if (b0 & 0xC0 != 0) return null; // delta overflowed, values meaningless
