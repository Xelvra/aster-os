...
const Layout = struct {
    ...
    fn writeRsdp(self: *Layout) void {
        @memcpy(self.buf[offFrom(rsdp_phys) .. offFrom(rsdp_phys) + 8], "RSD PTR ");
        @memcpy(self.buf[offFrom(rsdp_phys) + 10 .. offFrom(rsdp_phys) + 16], "ASTER0");
        self.writeU8(rsdp_phys, 15, 0); // revision 0
        self.writeU32(rsdp_phys, 16, @intCast(root_phys));
        self.fixChecksum(rsdp_phys, 8, 20);
    }

    fn writeRsdpV2(self: *Layout) void {
        @memcpy(self.buf[offFrom(rsdp_phys) .. offFrom(rsdp_phys) + 8], "RSD PTR ");
        @memcpy(self.buf[offFrom(rsdp_phys) + 10 .. offFrom(rsdp_phys) + 16], "ASTER0");
        self.writeU8(rsdp_phys, 15, 2); // revision 2
        self.writeU32(rsdp_phys, 16, 0);
        self.writeU32(rsdp_phys, 20, 36); // length
        self.writeU64(rsdp_phys, 24, root_phys);
        self.fixChecksum(rsdp_phys, 32, 36); // extended checksum
    }
    ...
    fn writeRoot(self: *Layout, xsdt: bool, table_addrs: []const u64) void {
        ...
        _ = self.writeHeader(root_phys, sig, length);
        for (table_addrs, 0..) |addr, i| {
            const off = 36 + i * entry_size;
            if (xsdt) {
                self.writeU64(root_phys, off, addr);
            } else {
                self.writeU32(root_phys, off, @intCast(addr));
            }
        }
        self.fixChecksum(root_phys, 9, length);
    }
    ...
    fn writeMadt(self: *Layout, entries: []const []const u8) void {
        ...
        _ = self.writeHeader(madt_phys, "APIC", length);
        self.writeU32(madt_phys, 36, 0xFEE00000); // local APIC address
        self.writeU32(madt_phys, 40, 0); // flags
        var off: usize = 44;
        for (entries) |e| {
            @memcpy(self.buf[offFrom(madt_phys) + off .. offFrom(madt_phys) + off + e.len], e);
            off += e.len;
        }
        self.fixChecksum(madt_phys, 9, length);
    }
